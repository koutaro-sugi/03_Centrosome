...
import {
  FlightLocation,
  CreateFlightLocationInput,
  UpdateFlightLocationInput,
} from "../types/flightLocation";
...
export const flightLocationAPI = {
  ...
  async create(
    userId: string,
    input: CreateFlightLocationInput
  ...
  async listByUser(userId: string, limit = 100): Promise<FlightLocation[]> {
    console.log(
      "[flightLocationAPI.listByUser] Fetching locations for user:",
      userId
    );
    ...
    console.log(
      "[flightLocationAPI.listByUser] Found locations:",
      locations.length
    );
  ...
  async incrementUsage(userId: string, locationId: string): Promise<void> {
    ...
    const command = new UpdateCommand({
      ...
      UpdateExpression:
        "SET usageCount = usageCount + :inc, lastUsedAt = :now, updatedAt = :now",
      ExpressionAttributeValues: {
        ":inc": 1,
        ":now": now,
      },
  ...
  async update(
    userId: string,
    input: UpdateFlightLocationInput
  ): Promise<void> {
