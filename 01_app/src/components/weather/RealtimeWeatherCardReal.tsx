/**
 * RealtimeWeatherCardReal コンポーネント
 * 実センサーデータ（AppSync GraphQL経由）からのリアルタイム気象データを表示
 */
...
/**
 * RealtimeWeatherCardRealのプロパティ
 */
interface RealtimeWeatherCardRealProps {
  /** デバイスID */
  deviceId: string;
  /** カードのタイトル */
  title?: string;
 ...
 * データ品質チェック
 */
const checkDataQuality = (data: SensorData): 'good' | 'warning' | 'error' => {
  // 温度範囲チェック
  ...
  // 気圧範囲チェック
  if (data.pressure !== undefined && (data.pressure < 900 || data.pressure > 1100)) {
    return 'warning';
  }
  
  // 風速範囲チェック
  if (data.windSpeed !== undefined && (data.windSpeed < 0 || data.windSpeed > 100)) {
    return 'warning';
  }
 ...
 * 異常な単位値を適切な値に変換
 */
const convertSensorData = (data: SensorData): SensorData => {
  const converted = { ...data };
  
  // 温度: 1000.7 → 100.07℃（10で割る）
  if (converted.temperature !== undefined && converted.temperature > 100) {
    converted.temperature = converted.temperature / 10;
  }
  
  // 風速: 615.4 → 61.54 m/s（10で割る）
  if (converted.windSpeed !== undefined && converted.windSpeed > 100) {
    converted.windSpeed = converted.windSpeed / 10;
  }
  
  // 風向: 5837 → 77度（360で余りを取る）
  if (converted.windDirection !== undefined && converted.windDirection > 360) {
    converted.windDirection = converted.windDirection % 360;
  }
  
  // 体感温度: 999.7 → 適切な値に変換
  if (converted.feelsLike !== undefined && converted.feelsLike > 100) {
    converted.feelsLike = converted.feelsLike / 10;
  }
 ...
 * RealtimeWeatherCardReal コンポーネント
 */
export const RealtimeWeatherCardReal: React.FC<RealtimeWeatherCardRealProps> = ({
  deviceId,
  title = '実センサー気象データ',
  height = 400,
  ...
  // データ変換
  const data = rawData ? convertSensorData(rawData) : null;
  const dataQuality = data ? checkDataQuality(data) : null;
